"""Sprint F: clinical-safety benchmark with Sprint F enhancements.

Sprint F additions vs Sprint E:
  - SE vocab expansion 605 → 659 (+54 biologic-specific AEs incl. CRS,
    ICANS, HLH, SCAR subtypes, ADC-payload effects, irAE endocrinopathies)
  - Curated priors updated to use new specific UMLS codes (230 → 324 pairs)
  - Reactome pathway-mediated edges as 8th α source (202 edges, 60 targets)
  - OnSIDES re-ingested on expanded vocab (504k → 548k non-zero pairs)
  - HPO-based AE rollup matcher (predicting parent AE counts as hit on
    specific child AE in benchmark)
  - Benchmark v4: 51 cases augmented with new specific UMLS codes

Eval reports BOTH strict-match AND HPO-rollup-match metrics.
"""
...
from ..data.clinical_safety_benchmark_v4 import EXPANDED_SAFETY_CASES_V4
...
def best_causal_rank(ranked: list[str], causal_umls: tuple[str, ...]) -> int | None:
    """Strict match: lowest rank where any causal UMLS appears."""
    causal_set = set(causal_umls)
...
def best_causal_rank_with_rollup(
    ranked: list[str], causal_umls: tuple[str, ...],
    rollup_map: dict[str, list[str]],
) -> int | None:
    """HPO rollup-aware match: predicted AE is parent of ground-truth AE.

    rollup_map: {umls_child: [umls_parents]}
    A prediction P at rank r matches if:
      - P is in causal_umls (strict), OR
      - any causal U has P in its rollup_map[U] (P is ancestor of U)
    """
    causal_set = set(causal_umls)
    ancestor_set: set[str] = set()
    for u in causal_umls:
        ancestor_set.update(rollup_map.get(u, []))
    full_match_set = causal_set | ancestor_set
    for i, s in enumerate(ranked):
        if s in full_match_set:
            return i + 1
    return None
...
def _process_safety_case(
    ...
    llm_blind_rank_rollup = best_causal_rank_with_rollup(
        llm_blind.ranked_side_effects, cf.causal_side_effects_umls, rollup_map,
    )
    ...
    llm_name_rank_rollup = best_causal_rank_with_rollup(
        llm_name.ranked_side_effects, cf.causal_side_effects_umls, rollup_map,
    )
    ...
    return {
        ...
        "llm_drug_blind_rank_rollup": llm_blind_rank_rollup,
        "llm_with_name_rank": llm_name_rank,
        "llm_with_name_rank_rollup": llm_name_rank_rollup,
...
def _report_table(records: list[dict], label: str, rollup: bool = False):
    n = len(records)
    suffix = "_rollup" if rollup else ""
    suffix_label = " (HPO rollup)" if rollup else " (strict)"
    print(f"\n--- {label}{suffix_label} (n={n}) ---")
    print(f"  {'method':<22s} {'hit@3':<14s} {'hit@10':<14s} "
          f"{'hit@20':<14s} {'hit@50':<14s}")
    for lbl, base_key in [("SCM (Sprint F α)", "scm_rank"),
                            ("RF-ECFP", "rf_ecfp_rank"),
                            ("LLM-drug-blind", "llm_drug_blind_rank"),
                            ("LLM-with-name", "llm_with_name_rank"),
                            ("Hybrid (Sprint F)", "hybrid_rank")]:
        # RF-ECFP doesn't have rollup variant
        key = base_key + suffix if base_key != "rf_ecfp_rank" else base_key
...
def main() -> int:
    print("=" * 78)
    print("Sprint F: Coverage Tier-1 Expansion benchmark")
    print("  + SE vocab 605→659 (+54 biologic AE terms)")
    print("  + Curated priors 230→324 (CRS proper, ICANS, HLH, SCAR, etc.)")
    print("  + Reactome pathway-mediated edges (8th source, 202 edges, 60 targets)")
    print("  + OnSIDES re-ingested on expanded vocab (504k→548k pairs)")
    print("  + HPO-based AE rollup matcher")
    print("  + Benchmark v4: 51 cases augmented with new specific UMLS codes")
    print("=" * 78)

    with open(RESULTS / "scm_edges_blended_f.json") as f:
        edges = json.load(f)
    ...
    print(f"[setup] HPO rollup: {len(rollup_map):,} UMLS codes with ancestors")

    target_set = set(target_info.keys())
    vocab_set = set(se_vocab)
    eligible = [c for c in EXPANDED_SAFETY_CASES_V4
    ...
    with open(RESULTS / "sprint_f_safety.json", "w") as f:
        json.dump({"n_cases": len(results), "per_drug": results}, f, indent=2)
    print(f"[save] {RESULTS / 'sprint_f_safety.json'}")

    # PRIMARY signal — strict match
    print("\n" + "=" * 78)
    print("Sprint F PRIMARY (full expanded benchmark, STRICT match)")
    print("=" * 78)
    _report_table(results, "Overall (full)", rollup=False)
    ...
    print(f"\n[full strict] McNemar Hybrid > LLM-drug-blind hit@10: "
          f"b={b} c={c} p={p_one:.4g}")

    lo, hi = _bootstrap_ci(results, "hybrid_rank", 10)
    h10 = hit(results, "hybrid_rank", 10)
    print(f"[full strict] Hybrid hit@10: {h10}/{len(results)} "
          f"({h10/len(results):.1%}) [95%CI: {lo:.1%}-{hi:.1%}]")

    # SECONDARY signal — HPO rollup
    print("\n" + "=" * 78)
    print("Sprint F SECONDARY (full expanded benchmark, HPO ROLLUP)")
    print("=" * 78)
    _report_table(results, "Overall (full)", rollup=True)

    h10r = hit(results, "hybrid_rank_rollup", 10)
    lor, hir = _bootstrap_ci(results, "hybrid_rank_rollup", 10)
    print(f"\n[full rollup] Hybrid hit@10 (rollup): {h10r}/{len(results)} "
          f"({h10r/len(results):.1%}) [95%CI: {lor:.1%}-{hir:.1%}]")

    # Sprint E paired comparison (strict)
    sprint_e_path = RESULTS / "sprint_e_safety.json"
    if sprint_e_path.exists():
        with open(sprint_e_path) as f:
            se = json.load(f)
        se_ids = {r["drug_id"] for r in se.get("per_drug", [])}
        e_intersect = [r for r in results if r["drug_id"] in se_ids]
        print("\n" + "=" * 78)
        print(f"Sprint F vs E: paired intersection (n={len(e_intersect)}, strict)")
        print("=" * 78)
        _report_table(e_intersect, "Sprint E intersection")

        # Pair-by-drug for McNemar
        e_by_id = {r["drug_id"]: r for r in se.get("per_drug", [])}
        pair_rows = [(r, e_by_id[r["drug_id"]]) for r in results
                     if r["drug_id"] in e_by_id]
        b_e = sum(1 for cur, ext in pair_rows
                  if (cur["hybrid_rank"] is not None and cur["hybrid_rank"] <= 10)
                  and not (ext["hybrid_rank"] is not None and ext["hybrid_rank"] <= 10))
        c_e = sum(1 for cur, ext in pair_rows
                  if not (cur["hybrid_rank"] is not None and cur["hybrid_rank"] <= 10)
                  and (ext["hybrid_rank"] is not None and ext["hybrid_rank"] <= 10))
        p_e = _mcnemar_one_sided(b_e, c_e)
        print(f"\n[F vs E paired strict] McNemar Hybrid hit@10: "
              f"b={b_e} c={c_e} p={p_e:.4g}")
    ...
    for ta in ("Oncology", "Immunology", "Cardiovascular & metabolic",
        ...
        if not sub:
            continue
        _report_table(sub, f"[{ta}]", rollup=False)

    # Per-TA — rollup
    print(f"\n--- Per-therapeutic-area (HPO ROLLUP) ---")
    for ta in ("Oncology", "Immunology", "Cardiovascular & metabolic",
               "CNS & psychiatry", "Other"):
        sub = [r for r in results if r["therapeutic_area"] == ta]
        if not sub:
            continue
        _report_table(sub, f"[{ta}]", rollup=True)
    ...
    summary = {
        ...
        "full_rollup": {
            "hits": {
                ...
                for label, key in [
                    ("scm", "scm_rank_rollup"),
                    ("hybrid", "hybrid_rank_rollup"),
                    ("llm_drug_blind", "llm_drug_blind_rank_rollup"),
                    ("llm_with_name", "llm_with_name_rank_rollup"),
                ]
            },
            "hybrid_hit10_bootstrap_95ci": [lor, hir],
        },
    }
    with open(RESULTS / "sprint_f_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\n[save] {RESULTS / 'sprint_f_summary.json'}")
    return 0
