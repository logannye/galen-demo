"""Sprint J: 4-track eval with router + confidence + n=120 OOD.

Two arms (Sonnet 4.6 and Opus 4.7) on:
  - Main n=224 (benchmark v5)
  - OOD n=120 (benchmark ood_v3)

Outputs include per-prediction confidence scores + router decisions.

Usage:
  python -m scripts.pipeline.run_sprint_j --llm sonnet
  python -m scripts.pipeline.run_sprint_j --llm opus
"""
...
def _process_safety_case(
    ...
    hybrid_j_ranked, promotions = apply_curated_prior_override_v2(
    ...
    hybrid_rank = best_causal_rank(hybrid_j_ranked, cf.causal_side_effects_umls)
    hybrid_rank_rollup = best_causal_rank_with_rollup(
        hybrid_j_ranked, cf.causal_side_effects_umls, rollup_map,
    )
    ...
    return {
        ...
        "hybrid_top10": hybrid_j_ranked[:10],
...
def calibration_metrics(results: list[dict]) -> dict:
    """Compute Brier + ECE on per-prediction confidence vs hit labels.

    For each prediction in top-10, label is 1 if predicted UMLS is in
    causal_side_effects_umls, else 0.
    """
    probs, labels = [], []
    for r in results:
...
def main() -> int:
    ...
    print("=" * 78)
    print(f"Sprint J: 4-track scaled ({llm_name.upper()} arm)")
    print("  + J.1: per-prediction confidence")
    print("  + J.2: OOD n=120")
    print("  + J.3: router-aware (novelty detected)")
    print(f"  + J.4: {llm_name.upper()} LLM arm")
    print("=" * 78)
    ...
    calibrator_coeffs = load_calibrator()
    print(f"[setup] calibrator: {calibrator_coeffs}")
    ...
    main_path = RESULTS / f"sprint_j_safety_{llm_name}.json"
    ...
    print(f"[main {llm_name}] Hybrid hit@10: {h10}/{len(main_results)} "
          f"({h10/len(main_results):.1%}) [95%CI: {lo:.1%}-{hi:.1%}]")

    main_calib = calibration_metrics(main_results)
    print(f"[main {llm_name} CALIBRATION] Brier={main_calib.get('brier', 0):.4f} "
          f"ECE={main_calib.get('ece', 0):.4f} "
          f"mean_conf={main_calib.get('mean_confidence', 0):.3f} "
          f"base_rate={main_calib.get('base_rate_hit', 0):.3f}")

    # OOD v3 (n=120)
    print("\n\n" + "=" * 78)
    print(f"STAGE 2: OOD-v3 EXPANDED ({llm_name}, n=120 target)")
    print("=" * 78)
    ...
    if ood_results:
        ...
        print(f"[OOD {llm_name} CALIBRATION] Brier={ood_calib.get('brier', 0):.4f} "
    ...
    summary_path = RESULTS / f"sprint_j_summary_{llm_name}.json"
